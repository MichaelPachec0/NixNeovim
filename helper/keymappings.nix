# { lib, cfg, helpers, ... }:
{ lib, customOptions, toLuaObject }:

let
  inherit (builtins) isString isAttrs;
  inherit (lib) filterAttrs assertMsg mapAttrsToList mkOption optionalAttrs;
  ...
  mapOption = submodule {
    options = {
      silent = boolOption false
        "Whether this mapping should be silent. Equivalent to adding <silent> to a map.";
      nowait = boolOption false
        "Whether to wait for extra input on ambiguous mappings. Equivalent to adding <nowait> to a map.";
      script = boolOption false "Equivalent to adding <script> to a map.";
      expr = boolOption false
        "Means that the action is actually an expression. Equivalent to adding <expr> to a map.";
      unique = boolOption false
        "Whether to fail if the map is already defined. Equivalent to adding <unique> to a map.";
      noremap = boolOption false
        "Whether to use the 'noremap' variant of the command, ignoring any custom mappings on the defined action. It is highly advised to keep this on, which is the default.";
      action = strNullOption "The action to execute";
      replace_keycodes = boolOption true
        "If expr is true, replace keycodes in the resulting string.";
      # actionLua   = strNullOption "The lua function to execute";
      desc = strNullOption
        "A textual description of this keybind, to be shown in which-key (or similar plugins).";
  ...
  genMaps = mode: maps:
    let
      normalizedMaps =
        builtins.mapAttrs (key: action: normalizeRhs action) maps;

      # filter inactive options
      filterActive = attrs: filterAttrs (_: v: v) attrs;

    in mapAttrsToList (key: mapping: {
      mode = mode;
      key = key;
      ...
      } // optionalAttrs (mapping.expr && !mapping.replace_keycodes)
      # Only write replace_keycodes when expr is true
        { inherit (mapping) replace_keycodes; };
    }) normalizedMaps;

  genAllMaps = mappings:
    (genMaps "" mappings.normalVisualOp) ++ (genMaps "n" mappings.normal)
    ++ (genMaps "i" mappings.insert) ++ (genMaps "v" mappings.visual)
    ++ (genMaps "x" mappings.visualOnly) ++ (genMaps "s" mappings.select)
    ++ (genMaps "t" mappings.terminal) ++ (genMaps "o" mappings.operator)
    ++ (genMaps "l" mappings.lang) ++ (genMaps "!" mappings.insertCommand)
...
in {
  ...
  luaString = mappings:
    let
      ...
      string = forEach (genAllMaps mappings) ({ mode, key, action, config }:
        ''
          do vim.keymap.set("${mode}", "${key}", ${action}, ${
            toLuaObject config
          }) end'');
    in concatStringsSep "\n" string;
